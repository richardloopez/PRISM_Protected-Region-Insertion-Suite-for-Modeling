...
def read_sequences_from_ali_temp(ali_file: str) -> Dict[str, str]:
    """
    Read all aligned sequences (including gaps) from a PIR/ALI file.

    Args:
        ali_file: Path to PIR alignment file

    Returns:
        Dictionary mapping align_code to sequence string
    ...
    try:
            ...
            for line in lines:
                ...
                if line_stripped.startswith('>P1;'):
                    if current_code and current_raw_sequence:
                        ...
                        sequences_map[current_code] = cleaned_sequence.split('*')[0]
                ...
                if current_code and line_stripped:
                    if line_stripped.startswith(('structureX', 'structure', 'sequence', 'CDE:', '#')):
                        continue
        ...
        if not sequences_map:
            raise ValueError("No sequences found in ALI file.")
...
def extract_cde_line(ali_file_path: str) -> str:
    ...
    Args:
        ali_file_path: Path to ALI file

    Returns:
        CDE line as string, or empty string if not found
    """
    try:
...
def add_cde_line_to_pir(clean_ali_path: str, cde_ali_path: str,
    ...
    Args:
        clean_ali_path: Path to clean PIR file (input)
        cde_ali_path: Path to output PIR file with CDE line
        ss2_file_path: Path to PSIPRED SS2 file
        target_seq_full: Complete target sequence
        align_code_sequence: Alignment code for the target sequence
        ...
        if char_in_alignment == '-':
            ...
            if ss_index < len(ss_string_full):
                ...
                raise IndexError(f"Error generating CDE: Alignment {align_code_sequence} (len {len(aligned_target_seq_temp)}) "
                                 f"appears longer than SS2 sequence (len {len(ss_string_full)}).")
    ...
    for line in pir_content_modeller:
        ...
        if line.startswith(f"sequence:{align_code_sequence}:"):

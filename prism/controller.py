...
0.5. (Optional) PSIPRED secondary structure prediction
...
from .config import (
    ...
    PERFORM_PSIPRED_PREDICTION, REFINE_FLANKS_DURING_AUTOMODEL
)
...
def main_workflow():
    ...
    print(f"\n{'='*80}")
    print(f"[STEP 0.5] Secondary Structure Prediction (PSIPRED)")
    print(f"{'='*80}\n")
    
    if PERFORM_PSIPRED_PREDICTION:
        print(f"[INFO] PERFORM_PSIPRED_PREDICTION = True. Requesting new .ss2 file from server...")
        try:
            psipred_client.run_psipred_request()
            print("\n[STEP 0.5] ✓ PSIPRED prediction complete.")
        except Exception as e:
            print(f"\n[FATAL ERROR] PSIPRED request failed: {e}")
            print("Please run PSIPRED manually and place the .ss2 file in the 'input/' directory.")
            job.wait() # Ensure parallel jobs are terminated
            sys.exit(1)
    else:
        print(f"[INFO] Skipping PSIPRED prediction (PERFORM_PSIPRED_PREDICTION = False).")
        print(f"       Checking for existing file: {SS2_FILE_PATH}")
        if not os.path.exists(SS2_FILE_PATH):
            print(f"\n[FATAL ERROR] PSIPRED file not found: {SS2_FILE_PATH}")
            print("Set PERFORM_PSIPRED_PREDICTION = True in config.py to run it automatically,")
            print("or provide the file manually in the 'input/' directory.")
            job.wait() # Ensure parallel jobs are terminated
            sys.exit(1)
        print("[INFO] ✓ Found existing SS2 file.")
    ...
    try:
        if USE_MANUAL_ALIGNMENT:
            ...
            # Use ALI file from the input directory
            ali_file_for_automodel = MANUAL_ALIGNMENT_FILE
            # Generate the CDE file in the *output* directory
            ali_file_for_analysis = os.path.join(MODELING_RESULTS_DIR, os.path.basename(MANUAL_ALIGNMENT_CDE_FILE))
            ...
            print(f"  > File for AutoModel (clean): {ali_file_for_automodel}")
            print(f"  > Generating Analysis file (with CDE) at: {ali_file_for_analysis}")
    ...
    # --- STEP 2: IDENTIFY EXPERIMENTAL REGIONS ---
    print(f"\n{'='*80}")
    ...
    try:
        ...
        experimental_ranges = utils.group_ranges(list(experimental_residues))
        ...
        loop_ranges_to_refine = utils.group_ranges(list(refinable_residues))

        range_strings = [f"[{s}-{e}]" if s != e else f"[{s}]" for s, e in loop_ranges_to_refine]
        print(f"\n[STEP 3] Final loop ranges to refine: {range_strings}")
        
        # --- STEP 3.5: Configure AutoModel Fixation Mode ---
        print(f"\n[STEP 3.5] Configuring fixation mode for AutoModel...")
        if REFINE_FLANKS_DURING_AUTOMODEL:
            # MODE: Optimize Flanks
            # Pass only the "deep core" to AutoModel.
            fixed_residues_for_automodel = truly_fixed_residues
            print(f"  > MODE: REFINE_FLANKS_DURING_AUTOMODEL = True")
            print(f"  > AutoModel WILL optimize flanks.")
            print(f"  > {len(fixed_residues_for_automodel)} 'deep core' residues will remain fixed.")
        else:
            # MODE: Freeze Flanks (Default)
            # Pass the complete experimental set to AutoModel.
            fixed_residues_for_automodel = experimental_residues
            print(f"  > MODE: REFINE_FLANKS_DURING_AUTOMODEL = False")
            print(f"  > AutoModel WILL freeze flanks (default behavior).")
            print(f"  > {len(fixed_residues_for_automodel)} residues (core + flanks) will remain fixed.")
    ...
    initial_models_names = homology_modeling.run_automodel(
        ...
        job,
        fixed_residues_for_automodel
    )

    # --- STEP 5: LOOP REFINEMENT ---
    print(f"\n{'='*80}")
    ...
    job.wait() # Explicitly wait for all jobs to finish
    ...
    if best_final_model:
        ...
        print(f"              'C' loops and 'C' flanks (flank={EXPERIMENTAL_FLANK_SIZE} res) were refined.")
        print(f"{'='*80}\n")

...
def submit_job(fasta_path: Path, email: str) -> str:
    ...
    '''
    logger.info(f"Submitting {fasta_path.name} to server...")
    ...
    logger.info(f"Job submitted successfully. UUID: {uuid}")
    return uuid
...
def run_psipred_request():
    ...
    if ss2_path:
        ...
        logger.info(f"Copied {ss2_path.name} to {target_ss2_path}")
    else:
